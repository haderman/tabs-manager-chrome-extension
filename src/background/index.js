...
async function loadData() {
  ...
  const result = fp.zipObject(ids, tabs);
...
async function checkOpenedWindows() {
  ...
  windowsOpenedIDs.forEach(async (id) => {
    ...
    modelsByWindowsID[id] = {
      ...
      numTabs: await api.Tabs.get(id).then(fp.length),
    };
  });
    ...
    setModel({
      ...
      modelsByWindowsID: {
        ...
        [tab.windowId]: {
          ...
          numTabsOpening:
            (model.modelsByWindowsID[tab.windowId].numTabsOpening - 1),
        },
      },
    });
        ...
        setModel({
          ...
          modelsByWindowsID: {
            ...
            [tab.windowId]: {
              state: machine.getCurrentState(),
              workspaceInUse:
                model.modelsByWindowsID[tab.windowId].workspaceInUse,
              numTabs: fp.length(tabs),
            },
          },
        });
      });
...
function handleTabChanges(windowId) {
  api.Tabs.get(windowId).then((tabs) => {
    updateCountTabsInModel(windowId)(fp.length(tabs));

    const machine = model.machinesByWindowsID[windowId];
    const windowModel = model.modelsByWindowsID[windowId];
    const workspace = model.data[windowModel.workspaceInUse];

    api.Workspaces.update(workspace, tabs).then(
      function updateDataAfterOnTabCreated([id, dataSaved]) {
...
function handleOnMessages(request, sender, sendResponse) {
  ...
  if (
    ...
    machine.isEventAvailable("CREATE_WORKSPACE")
  ) {
      ...
      setModel({
        ...
        modelsByWindowsID: {
          ...
          [window.id]: {
            state: machine.getCurrentState(),
            workspaceInUse: id,
            numTabs: fp.length(dataSaved[id].tabs),
          },
        },
      });
    }
  ...
  if (
    ...
    machine.isEventAvailable("UPDATE_WORKSPACE")
  ) {
  ...
  if (
    ...
    machine.isEventAvailable("DELETE_WORKSPACE")
  ) {
  ...
  if (
    ...
    machine.isEventAvailable("DISCONNECT_WORKSPACE")
  ) {
...
async function openWorkspace(workspaceId, window) {
  const getWorkspaceInUse = fp.compose(
    fp.prop("workspaceInUse"),
    fp.prop(window.id),
    fp.prop("modelsByWindowsID"),
  );
  ...
  const tabsToOpen = await api.Workspaces.get(workspaceId)
    .then(fp.prop("tabs"))
    .then(fp.map(fp.prop("url")))
    .then(fp.map((url) => ({ windowId: window.id, url })))
    .catch((err) => {
  ...
  setModel({
    ...
    modelsByWindowsID: {
      ...
      [window.id]: {
        ...
        numTabsOpening: fp.length(tabsToOpen),
        numTabsClosing: fp.length(currentlyOpenTabs),
      },
    },
  });

  await api.Tabs.create(tabsToOpen);
  await api.Tabs.remove(currentlyOpenTabs.map(fp.prop("id")));
